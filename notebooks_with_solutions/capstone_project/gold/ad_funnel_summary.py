...
TABLE_NAME = "local.gold.ad_funnel_summary"
...
def transform(input_dfs: dict[str, DataFrame]) -> DataFrame:
    """
    Aggregate obt_ad to ad_group × day grain.
    Metrics:
    ...
    Rolling windows (l7d, l30d, l90d) on cvr.
    """
    ...
    daily = (
        ...
        .withColumn(
            ...
            F.when(
                ...
                F.col("total_conversions") / F.col("total_clicks"),
            ).otherwise(F.lit(None)),
        )
        .withColumn(
            "cpa",
            F.when(
                ...
                F.col("total_spend") / F.col("total_conversions"),
            ).otherwise(F.lit(None)),
        )
    )

    window_7d = (
        Window.partitionBy("ad_group_id")
    ...
    return daily.select(
        ...
        F.col("cpa"),
        (
            F.sum("total_conversions").over(window_7d)
            / F.nullif(F.sum("total_clicks").over(window_7d), F.lit(0))
        ).alias("l7d_cvr"),
        (
            F.sum("total_conversions").over(window_30d)
            / F.nullif(F.sum("total_clicks").over(window_30d), F.lit(0))
        ).alias("l30d_cvr"),
        (
            F.sum("total_conversions").over(window_90d)
            / F.nullif(F.sum("total_clicks").over(window_90d), F.lit(0))
        ).alias("l90d_cvr"),
    )
