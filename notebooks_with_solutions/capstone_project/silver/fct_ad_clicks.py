...
TABLE_NAME = "local.silver.fct_ad_clicks"
...
def extract(
    ...
    return {
        "ad_click": spark.table("local.bronze.ad_click").filter(
...
def transform(input_dfs: dict[str, DataFrame]) -> DataFrame:
    return input_dfs["ad_click"].select(
        F.col("click_id"),
        F.col("impression_id"),
        F.col("session_id"),
        F.col("cost"),
        F.col("clicked_at"),
        F.col("created_at"),
        (F.col("cost") * 100).cast("long").alias("cost_in_cents"),
    )
