...
@dag(
    dag_id="silver.fct_order_lines_multi_upstream_et",
    schedule=[
        bronze_order_lines,
        bronze_warehouse,
    ],  # <-- triggered when boths assets have been update atleast once each
    start_date=pendulum.datetime(2025, 1, 1, tz="UTC"),
    catchup=True,
    max_active_runs=1,  # only one run at a time, in order
    tags=["silver", "order_lines", "event-driven", "multi-event-driven"],
)
def silver_fct_order_lines_pipeline():
    @task(retries=2, retry_delay=2, inlets=[bronze_order_lines, bronze_warehouse])
